...
class XLerobot2Wheels(Robot):
    ...
    def _body_to_wheel_raw(
        ...
        # Correct for wheel mounting direction before commands reach the motors.
        left_wheel_raw = self._degps_to_raw(left_wheel_degps) * self.config.left_wheel_direction
        right_wheel_raw = self._degps_to_raw(right_wheel_degps) * self.config.right_wheel_direction
    ...
    def _wheel_raw_to_body(
        ...
        # Use the same correction for feedback so observed body velocity remains coherent.
        left_degps = self._raw_to_degps(left_wheel_speed) * self.config.left_wheel_direction
        right_degps = self._raw_to_degps(right_wheel_speed) * self.config.right_wheel_direction
