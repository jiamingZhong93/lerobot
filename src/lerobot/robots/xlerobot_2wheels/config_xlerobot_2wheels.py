...
class XLerobot2WheelsConfig(RobotConfig):
    ...
    port1: str = "/dev/xlerobot_left"  # port to connect to the bus (so101 + head camera), /dev/ttyACM1
    port2: str = "/dev/xlerobot_right"  # port to connect to the bus (arms + 2 wheels), /dev/ttyACM0
    ...
    # Physical mounting may invert a wheel's positive rotation. Keep these at 1 for
    # the standard wiring; set the affected wheel to -1 when its direction is reversed.
    left_wheel_direction: int = 1
    right_wheel_direction: int = 1
