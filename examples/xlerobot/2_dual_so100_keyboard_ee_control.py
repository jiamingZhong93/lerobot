            ...
            if keyboard_action:
                ...
                for key, value in keyboard_action.items():
                    ...
                    arm2_joint_controls = {
                        ...
                        'l': ('wrist_roll', -1),      # Joint5 decrease
...
def main():
    ...
    try:
        ...
        arm_left_port = "/dev/xlerobot_left"  # xlerobot_left (ttyACM1)
        arm_right_port = "/dev/xlerobot_right"  # xlerobot_right (ttyACM0)
        
        print(f"Configuring first (left) arm: {arm_left_port}")  
        print(f"Configuring second (right) arm: {arm_right_port}")
        ...
        arm_left_config = SO100FollowerConfig(port=arm_left_port, id="xlerobot_left", use_degrees=True)
        arm_right_config = SO100FollowerConfig(port=arm_right_port, id="xlerobot_right", use_degrees=True)
        
        arm_left_robot = SO100Follower(arm_left_config)
        arm_right_robot = SO100Follower(arm_right_config)
        
        robots = {
            'arm1': arm_left_robot,
            'arm2': arm_right_robot
        }
        ...
        # Connect devices
        print("Connecting first (left) arm...")
        arm_left_robot.connect()
        print("Connecting second (right) arm...")
        arm_right_robot.connect()
