...
# Smooth return-to-start parameters. The start pose is read before the startup zero command.
...
# The left base wheel is motor ID 9. Its mounting on this robot reverses the standard direction.
...
RIGHT_KEYMAP = {
    ...
    # J/L are reserved for differential-drive steering.
    ...
    'triangle': 'p',  # Rectangle trajectory key; avoid Shift+Y ambiguity with the left arm.
}
...
class SimpleHeadControl:
    ...
    def p_control_action(self, robot, kp=None, observation=None):
        obs = robot.get_observation() if observation is None else observation
        control_kp = self.kp if kp is None else kp
        action = {}
        for motor in self.target_positions:
            current = obs.get(f"{HEAD_MOTOR_MAP[motor]}.pos", 0.0)
            error = self.target_positions[motor] - current
            control = control_kp * error
            action[f"{HEAD_MOTOR_MAP[motor]}.pos"] = current + control
        return action

    def target_error(self, observation):
        return sum(
            abs(self.target_positions[motor] - observation.get(f"{HEAD_MOTOR_MAP[motor]}.pos", 0.0))
            for motor in self.target_positions
        )
...
class SimpleTeleopArm:
    def __init__(self, kinematics, joint_map, initial_obs, prefix="left", kp=0.81):
        ...
        self.start_positions = self.joint_positions.copy()
    ...
    def return_to_start_target(self):
        """Restore the joint pose captured before this script moved the arms to zero."""
        self.target_positions = self.start_positions.copy()
        # Preserve the returned wrist angle when handle_keys reapplies its wrist coupling.
        self.pitch = (
            self.start_positions["wrist_flex"]
            + self.start_positions["shoulder_lift"]
            + self.start_positions["elbow_flex"]
        )
...
def return_to_start_position(robot, left_arm, right_arm, head_control):
    """Return both arms and the head to the pose observed before startup zeroing."""
    print("[MAIN] Returning both arms and head to the startup position...")
    left_arm.return_to_start_target()
    right_arm.return_to_start_target()
    ...
    while time.monotonic() < deadline:
        ...
        action = {
            **left_arm.p_control_action(robot, kp=RETURN_KP, observation=observation),
            **right_arm.p_control_action(robot, kp=RETURN_KP, observation=observation),
            **head_control.p_control_action(robot, kp=RETURN_KP, observation=observation),
            "x.vel": 0.0,
            "theta.vel": 0.0,
        }
        robot.send_action(action)
        ...
        if total_error <= RETURN_POSITION_TOLERANCE:
            print("[MAIN] Startup position reached.")
            return

        precise_sleep(control_period)

    print("[MAIN] Startup return timed out; holding the closest reachable position.")
...
class SmoothBaseController:
    ...
    def update(self, pressed_keys, robot):
        ...
        # N/M adjust the speed level once per press, rather than once per control-loop iteration.
        newly_pressed_keys = pressed_keys - self.previous_pressed_keys
        self.previous_pressed_keys = pressed_keys
        if robot.teleop_keys["speed_up"] in newly_pressed_keys:
            robot.speed_index = min(robot.speed_index + 1, len(robot.speed_levels) - 1)
            print(f"[BASE] Speed level: {robot.speed_index + 1}/{len(robot.speed_levels)}")
        if robot.teleop_keys["speed_down"] in newly_pressed_keys:
...
def connect_robot(robot):
    """Retry transient Feetech no-status-packet errors without changing calibration."""
    for attempt in range(1, CONNECT_ATTEMPTS + 1):
        try:
            robot.connect()
            return
        except ConnectionError:
            # robot.connect() opens both buses before configure() enables torque. Close both
            # ports directly so the next attempt starts from a known serial state.
...
def main():
    ...
    # SO101Kinematics and all arm target values in this script use degrees.
    ...
    print(f"    {robot.teleop_keys['quit']}: Stop base, release all motors, and quit")
    ...
    print(f"    P: Execute rectangular trajectory")

    print("\n↩️  Return to Start:")
    print("    X: Smoothly return both arms and head to the pose at program startup")
    ...
    try:
        ...
        while True:
            ...
            # X is a one-shot global command. V now controls the left pitch decrease.
            if "x" in newly_pressed_keys:
                return_to_start_position(robot, left_arm, right_arm, head_control)
                continue

            if robot.teleop_keys["quit"] in newly_pressed_keys:
            ...
            # Handle rectangular trajectory for right arm (p key)
        ...
        try:
            if robot.is_connected:
                print("[MAIN] Stopping base, releasing all motor torque, and disconnecting...")
                robot.disconnect()
        finally:
            keyboard.disconnect()
